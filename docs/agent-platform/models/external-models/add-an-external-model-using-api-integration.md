...
You can connect an external model to (In)Agent Platform using API integration. This feature extends (In)Agent Platform's functionality by allowing you to bring in models from external sources.
...
Once the integration is successful and the inference toggle is ON, you can use the model across (In)Agent Platform. You can also turn inferencing OFF if needed.

...
* Select the appropriate application and click Select. After selecting the application, set the following retrieval configurations.
  ...
  * **Meta Filters** - This field configures how information is retrieved from the knowledge base — use it to set additional filters on content in Search AI. For example, to retrieve only content from ingested webpages:
...
The response includes both the script output and detailed logs. These logs provide visibility into the underlying Search AI operations and can be used to troubleshoot errors, analyze performance, and refine query behavior. You can update the configurations on the left-hand side — selecting Chunks, Answers, or both — and responses are generated accordingly.

![Test the Knowledge Base](images/test-knowledgebase-tool.png "Test the Knowledge Base")

When metadata filters are enabled and reference memory variables, the Metadata input field appears. Enter sample values in JSON format to allow the system to resolve those variables during testing and accurately simulate runtime behavior, for example:
    ...
    ```
Both the query text and metadata values persist across test runs, so you can iterate without re-entering them — they're cleared only when you explicitly modify or clear the fields. At runtime, variables are resolved automatically from the session context; if a variable is't found, it's treated as empty and execution continues without an error.

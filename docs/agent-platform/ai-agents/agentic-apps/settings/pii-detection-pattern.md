...
* Direct voice calls to Agent Platform : PII handling is not applied on audio when users interact with agents via voice channels.

* Voice via AI for Service Platform using ASR/TTS : When Agent Platform is integrated with AI for Service Platform and uses its ASR (speech-to-text) and TTS (text-to-speech), the existing PII detection policies and safety guardrails are applied on the text transcript produced by ASR.

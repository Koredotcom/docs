...
**Direct voice calls to Agent Platform**

  * When users interact with agents through real‑time voice, PII handling and guardrails are not applied to the audio stream. 
  * For these types of interactions, Agent Platform does not enforce PII handling or guardrails on the input or output. The user inputs are sent directly to the model. Any PII information shared by the user in this mode can be present in model processing and associated logs.
    
**Voice via AI for Service Platform using ASR/TTS**

 * When Agent Platform is integrated with AI for Service Platform and uses its ASR (speech‑to‑text) and TTS (text‑to‑speech), PII handling occurs on the text transcript generated by ASR.

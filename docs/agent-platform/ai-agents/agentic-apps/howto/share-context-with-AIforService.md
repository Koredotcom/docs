...
3. Configure metadata to be passed to AI for Service during the handoff. The event supports two types of parameters:
    ...
    * *System-defined parameters*: These parameters are explicitly defined. These parameters can have static values or dynamic values from the context or memory. 

        Examples:
        1. Static value
            * Add a parameter that indicates the source of information as ‘Agent Platform’.
            * "source": "Agent Platform" 
        2. Dynamic Values
            * Add parameters that pass the user details and order details from the conversation, stored in memory.

...
Whenever a user clicks on the Playground option, by default, a new user session is automatically created. Alternatively, click the **New Session** button at the top of the Playground interface to manually start a new session. This action:

* Clears the current conversation from the chat window.
* Initiates a new session with a blank slate.
* Does **not delete** previous conversation history (you can still access it via the **History** button)
...
:::image type="content" source="images/playground/session_debug.png" alt-text="Illustration shows the debug logs of the session." lightbox="images/playground/processing-timeline.png":::

...
5. Real-time Voice - Enable this feature to allow users to interact with the app through voice. If this feature is enabled, select the AI model to be used by the agent and supervisor for voice interactions. Refer to [this](../../../agent-platform/models/supported-models.md) for the list of supported models. Use the gear icon to modify the model settings. Note that the configuration properties vary by model. 

...
To finish configuring your directory, schedule auto sync for your AD. Enabling auto sync is **_optional_** but highly recommended to keep your AD data on (In)Agent Platform up-to-date. 

Auto sync will ensure that any changes in AD, such as user additions or deletions, are automatically reflected on (In)Agent Platform.
...
* **Reset**: Resets the AD sync configurations but retains the last sync data on (In)Agent Platform.

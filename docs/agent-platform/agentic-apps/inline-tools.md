...
**Name**: Enter a unique and descriptive name for the tool. 

**Description**:A comprehensive description of the tool's functionality. This is intended to guide the agent on the appropriate usage of the tool , when and how to use the tool.

**Added Parameters**: Add the necessary input parameters required by the tool to complete the intended operation. For each parameter, provide a unique name, a description, and the type of the parameter. The description helps the agent identify the appropriate entity from the user input.

**Definition**:  The definition of the tool is the code in JAvascript or Python that is executed when the tool is invoked with the parameters. 

**Response Preview:** This can be used to test the definition of the tool. Click on execute tool and provide the input parameters to view the response that is generated by executing the definition. 

Once the tool is defined and tested locally, click on Create Tool to add the tool to the Agent. 
 

To create an inline tool, click **+Create Tool** and provide the following details:

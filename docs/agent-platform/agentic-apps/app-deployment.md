...
Deploying an Agentic app transitions it from development to production, making it accessible to end users within specified environments. This document guides the management of application versions, setting up environments, and deploying apps using APIs.
...
* **Immutable Releases:** Prevents changes to deployed versions to ensure stability.

* **Draft Testing:** Fully test apps in draft mode before finalizing a version.

* **Selective Versioning:** Choose draft, previous, or specific versions of agents and tools.

* **Environment Isolation:** Deploy separate versions to dev, test, and prod environments.
...
The Environments feature enables users to configure separate deployment environments for their agents, supporting multiple environments such as development, testing, staging, and production throughout the application development and deployment lifecycle. Each environment has its isolated configurations.

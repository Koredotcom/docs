...
Authorize our application to perform cryptographic operations with your key by assigning the appropriate roles.

1. **Navigate to your Key Vault** in the Azure portal and select **Access control (IAM)**.
2. **Add role assignments** by selecting **Add role assignment** and choosing the appropriate roles for our application's service principal (search by our application name or service principal):
...
2. **Find and approve** our pending connection request

These steps ensure our application can securely access your Key Vault key for BYOK functionality without requiring your credentials or compromising security.

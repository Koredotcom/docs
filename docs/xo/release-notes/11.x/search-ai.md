...
Search AI now supports a Delete Chunks by ID API, letting you remove individual chunks from the search index without deleting the parent document. The API targets chunks using specified IDs and returns clear validation or partial success responses when inputs are invalid or only some IDs are matched. [Learn more  :octicons-arrow-right-24:](../../apis/searchai/chunk-apis.md#delete-chunks-by-id-api).

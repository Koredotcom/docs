...
5. Select **Enforce JTI**, **JWE**, or both as per your requirements to secure the connections with the Platform:
    ...
    * **Enforce Request and Response Payload Encryption**: Provide a symmetric encryption key generated by the platform to encrypt Public API request and response payloads, including Token Introspection and User Information data. Use the same symmetric encryption key to decrypt the payloads. The symmetric encryption key is securely exchanged by encrypting it with a public key and decrypting it with the corresponding private key. Refer [How Request and Response Payload Encryption Works](#how-request-and-response-payload-encryption-works).    

...
| [Outbound Calling (VG)](./outbound-calling-vg-v2.md)                                               | To trigger an outbound call and receive call status notifications. In v2, Automatic Machine Detection (AMD) configurations are sent when the API is invoked, enabling the detection process to start earlier. This improves performance and reduces latency compared with v1, where AMD commands are triggered only after the call is initiated.                                   | SmartAssist Dialout  |
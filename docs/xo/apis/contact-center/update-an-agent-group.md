...
To update a particular agent group with the given `IId` and `agentGroupId`.
...
```
curl --location --request POST 'https://{{host}}/agentassist/api/v1/public/{{IId}}/agent-groups/{{agentGroupId}}' \

...
To delete a specific queue with the given  `IId` and `queueId`.

| **METHOD**          | **DELETE**                                               |
|-----------------|------------------------------------------------------|
| **Endpoint**       | `https://{{host}}/agentassist/api/v1/public/{{IId}}/queues/{id}` |
...
```
curl --location --request DELETE 'http://{{host}}/agentassist/api/v1/public/{{IID}}/queues' \

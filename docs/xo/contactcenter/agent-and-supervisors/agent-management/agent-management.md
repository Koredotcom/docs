...
3. Click `Update` to save the changes. A confirmation message is displayed.  
...
1. The response time after which the status should trigger: This can be set in either as a percent value of a total or as minutes and seconds.
    ...
    2. The statuses that do not require percentile or response time configuration are: `On Connect`, `On Interruption`, and `On Close`.  

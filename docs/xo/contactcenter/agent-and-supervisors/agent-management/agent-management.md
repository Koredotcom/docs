...
By default, the blended agents functionality is disbaled. Administrators can turn on the toggle to enable the functionality, allowing agents to handle both voice and digital interactions concurrently. For mor information on the changes when blended mode is enabled see [Key Changes with Blended Mode](#key-changes-with-blended-agent-mode).  

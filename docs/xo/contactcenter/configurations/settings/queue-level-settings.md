...
* **Queue-level Outbound Numbers/Emails**: Turn on the toggle to enable the feature. Agents can see only the outbound phone numbers and email addresses configured for their respective queues.
* **Redirect Queued Conversations to Out of Business Hours Flow**: When the toggle is turned on, any conversation waiting in the queue is automatically redirected to the Out of Business Hours flow when the system enters Out of Business Hours.

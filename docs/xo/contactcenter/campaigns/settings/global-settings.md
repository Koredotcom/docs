...
Supervisors can configure the maximum count of concurrent calls for the campaigns. By default, the count is set to 10 and the maximum count that can be set is 15. This applies only to Agentless Campaigns. Progressive and Preview Campaigns are unaffected, as they depend solely on the number of available agents.

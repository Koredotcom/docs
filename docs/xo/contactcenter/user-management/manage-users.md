...
* Switch on **Enable chat** toggle to set:
   1. Capacity by Channel Type (Maximum handling capacity for individual channels)
   2. Language Support (Supported languages and their proficiency level)
   3. Widget Features (Attachments and emojis in live chat)

* Switch on **Enable Voice** toggle to set the supported languages and their proficiency level.

* Switch on **Enable Voice Mail** toggle to set the count of voice mails callers can leave in a queue.
...
1. Type in a queue name in the search field, then select the one you need from the list. The list only contains queues that are already created. To add a queue, see [Contact Center > Queues > Add Queues](https://docs.kore.ai/smartassist/routing/queues/).
...
1. Type in a skill name in the search field, and select it from the dropdown list to add it to the user’s skill list. The list only contains skills that are already created. To add more skills, see [Contact Center > Skills > Add Skills](https://docs.kore.ai/smartassist/skills/skills/).
...
!!! Notes
   ...
   4. After deleting a user record, the respective user can no longer sign into Contact Center AI. If you want to temporarily restrict users from handling customer conversations, you can turn off their access to chat and voice channels, or add them to the [Unassigned group](https://docs.kore.ai/smartassist/agent/agent-group/agent-groups/#Preconfigured_Groups).

# Add a Group

You can add a group to Contact Center AI by following the steps below:
...
To save the user, click the **Share Invites** button at the bottom of the New Groups window.

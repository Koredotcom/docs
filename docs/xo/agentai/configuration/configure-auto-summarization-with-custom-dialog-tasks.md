...
If clients prefer to use a different model (for reasons such as language or quality), they can host their model in [GALE](../../../gale/home.md){:target="_blank"} or another server and provide their API endpoint. The custom summarization workflow uses the "Custom Dialog Task" option - when the conversation ends, Agent AI will trigger the configured dialog, which will fetch the conversation, generate the summary, and display or save it as needed.

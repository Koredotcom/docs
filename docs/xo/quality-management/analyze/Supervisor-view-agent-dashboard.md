...
Each agent has a dedicated dashboard that is accessible to both the agent and their supervisor. It displays high-level metrics for supervisors to review. Supervisors can filter dashboard data by Language, [**Date Range**](../analyze/dashboard.md/#date-range) and [**Channel**](../analyze/dashboard.md/#channel). 

...
| Description | Impact |
|-------------|--------|
| Does your firewall support the pinhole feature? | Firewalls that perform Network Address Translation (NAT) map an external IP address and port to an internal IP address and port. This mapping is called a pinhole. |
| If your firewall supports the pinhole feature, does it support the following modules? <br><br> **NAT hole punching** <br> **NAT traversal** <br> **TCP hole punching** <br> **UDP hole punching** | **NAT hole punching:** Without support, the system cannot establish a direct connection between peers when one or both are behind firewalls or NAT-enabled routers. <br><br> **NAT traversal:** Without support, devices behind a NAT cannot communicate with external devices. NAT traversal enables direct communication without port forwarding or manual configuration. <br><br> **TCP hole punching:** Without support, the system cannot establish TCP connections between peers using NAT traversal. <br><br> **UDP hole punching:** Without support, the system cannot establish bidirectional UDP connections between peers using NAT traversal. |
| Can you enable WSS support on your firewall to establish a WebSocket over an encrypted TLS connection? | Without WSS support, the system cannot establish a secure WebSocket connection to transmit data securely over TLS. |
| Do you connect to the internet through a router? | If required ports are not opened as described in the firewall requirements, the system cannot establish peer connections and blocks traffic. |
| Do you use a proxy server to connect to the internet? | When a proxy server is in use, the system cannot perform direct NAT traversal with the end user. |
| If you use a proxy server, can you bypass it in the Kore configuration environment? | When you bypass the proxy, NAT traversal occurs directly with the end user, and data transmission proceeds smoothly. |
...
| Can you allow and bypass traffic for `*.kore.ai` and `*.twilio.com` domains (HTTPS) on ports 443, 8443, 3478, and 5349? | Without allowing this traffic, the system cannot establish connections, perform NAT traversal, or transmit audio and data packets between peers. |
| Can you whitelist and allow traffic to all required IP addresses and domains (SBCs, STUN/TURN, and others) as specified in the firewall requirements? | Without whitelisting, the system cannot establish connections, perform NAT traversal, or transmit IP packets between peers. |
| Can you allow all required UDP ports in a bidirectional manner as specified in the firewall requirements? | Without bidirectional UDP ports, the system cannot transmit data in both directions, which results in one-way audio issues. |
| Do you use any antivirus software? | Antivirus software can block traffic and must allow data through the required ports. |
| If you use antivirus software, does it require federation rules? | If federation rules apply, traffic must be allowed according to Kore requirements and local regulations. |
| Can you run the AWS latency test from the agent office premises and share the results? | Latency up to 150 ms is acceptable. Latency above 150 ms degrades call quality, and latency between 150 ms and 300 ms remains within acceptable limits. |
| Do you use an internal firewall on each agent system, such as Zscaler? | Required UDP ports must be opened. Otherwise, RTP audio packets cannot transmit between peers and audio traffic is blocked. |
| If you use an internal firewall such as Zscaler, can you allow all required UDP ports in a bidirectional manner? | Without bidirectional UDP ports, the system cannot transmit RTP audio packets and blocks audio traffic. |

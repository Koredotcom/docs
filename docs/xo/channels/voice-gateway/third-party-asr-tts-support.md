# **Support for Third-Party ASR, TTS, and Voice Biometrics**

## Automatic Speech Recognition (ASR)

Kore supports the following third-party service providers for [ASR services](../voice-gateway/configure-voice-gateway.md#voice-preferences):

| **Name**              | **Platform**                                                                                         | **Voice Gateway (VG) Support** | **Audiocodes Support** |
|-----------------------|------------------------------------------------------------------------------------------------------|-------------------------------|-------------------------|
| Google                | [Google Cloud](https://cloud.google.com/speech-to-text)                                              | ✅                            | ✅                      |
| Azure                 | [Azure AI Services](https://azure.microsoft.com/en-us/products/ai-services/ai-speech/#Features)      | ✅                            | ✅                      |
| Deepgram              | [Speech to text](https://deepgram.com/product/transcription)                                         | ✅                            | ❌                      |
| Advanced Media        | [AmiVoice](https://www.advanced-media.co.jp/english/aboutus/amivoice)                                | ❌                            | ✅                      |
| Amazon Transcribe     | [Amazon Transcribe](https://aws.amazon.com/pm/transcribe/)                                           | ✅                            | ❌                      |
| AmiVoice              | [AmiVoice Cloud Platform](https://acp.amivoice.com/amivoice/)                                        | ✅                            | ✅                      |

## Text to Speech (TTS)

Kore supports the following third-party service providers for [TTS services](../voice-gateway/configure-voice-gateway.md#voice-preferences):

| **Name**       | **Platform**                                                                                          | **Voice Gateway (VG) Support** | **Audiocodes Support** |
|----------------|-------------------------------------------------------------------------------------------------------|-------------------------------|-------------------------|
| Google         | [Cloud TTS](https://cloud.google.com/text-to-speech)                                                  | ✅                            | ✅                      |
| Azure          | [Azure AI Services – AI Speech](https://azure.microsoft.com/en-us/products/ai-services/ai-speech/#Features) | ✅                            | ✅                      |
| OpenAI         | [Open AI TTS](https://platform.openai.com/docs/guides/text-to-speech)                                 | ✅                            | ❌                      |
| Eleven Labs    | [Eleven Labs TTS](https://elevenlabs.io/text-to-speech)                                               | ✅                            | ❌                      |
| AWS            | [Amazon Polly](https://aws.amazon.com/polly/)                                                         | ✅                            | ✅                      |
| PlayHT         | [PlayHT TTS](https://play.ht/)                                                                         | ✅                            | ❌                      |

## Voice Biometrics

Kore supports the following third-party service providers for voice biometrics:

| **Name**  | **Platform Description/Details (Version)**                                                                                          | **Voice Gateway (VG) Support** | **Audiocodes Support** |
|-----------|--------------------------------------------------------------------------------------------------------------------------------------|-------------------------------|-------------------------|
| ID R&D    | [ID Voice](https://www.idrnd.ai/idvoice-verified-voice-biometrics-and-anti-spoofing/)                                               | ✅                            | ❌                      |
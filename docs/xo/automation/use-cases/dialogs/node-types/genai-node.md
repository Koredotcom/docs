...
2. Contextual Intents:
    1. Contextual intents (Dialog or FAQs) recognized from the user input continue to be honored as per the Interruption Settings defined in the app definition.
...
5. The platform stores the entity values in the context object, and this information can be used to define the transitions or any other part of the app configuration.
...
Large Language Models come with challenges, such as potentially unwanted behavior. This is why the Agent Node is built with the necessary infrastructure to enable you to define AI Agent behavior. The node considers existing entities and context and allows you to configure specific rules and exit scenarios that ensure the AI Agent acts within the boundaries of your business.
...
2. You can add the Agent Node just like any other node. You can find it in the main list of nodes, as it does not depend on the Action node.
...
This applies only to String and Description entities: Select one of these options to determine the course of action if the AI Agent encounters an entity as a part of the user utterance:
